...
def forecast_parameters_batched(
  ...
  num_batches = int(np.ceil(data_size / batchsize))
  for batch_id in range(num_batches):
    start = batch_id * batchsize
    end = min((batch_id + 1) * batchsize, data_size)
    if start >= end:
      continue
    batch_slice = slice(start, end)
    forecast_params = forecast_inner(params, features[batch_slice])
    for param_id, fc_param in enumerate(forecast_params):
      ...
      forecast_params_slices[param_id].append(fc_param)
...
def make_model(
  ...
  mlp = models.BayesianNeuralField1D(
      ...
      use_physical_guidance=use_physical_guidance,
      num_reference_points=num_reference_points,
      reference_feature_start=reference_feature_start,
      reference_feature_stride=reference_feature_stride,
      time_feature_index=time_feature_index,
      frequency_feature_index=frequency_feature_index,
      target_coord_indices=target_coord_indices,
      time_fourier_order=time_fourier_order,
      frequency_fourier_order=frequency_fourier_order,
      time_fourier_scale=time_fourier_scale,
      frequency_fourier_scale=frequency_fourier_scale,
  )
...
def fit_map(
  ...
  target_scale = float(np.nanstd(target))
...
def ensemble_vi(
  ...
  return (_make_surrogate_posterior(*surrogate_params, batch_ndims=2), losses, predictions,)
