...
def make_multiscale_fourier_features(
    x: jax.typing.ArrayLike,
    max_order: int,
    scale: float,
    rescale: bool = False,
) -> jnp.ndarray:
  """Returns paper-style multiscale sine/cosine features."""
  x = jnp.reshape(x, (-1, 1))
  if max_order <= 0:
    return jnp.zeros((x.shape[0], 0))
  orders = jnp.arange(max_order, dtype=x.dtype)
  theta = (2.0 ** orders) * scale * x
  features = jnp.stack((jnp.sin(theta), jnp.cos(theta)), axis=-1)
  features = jnp.reshape(features, (features.shape[0], -1))
  denominator = jnp.repeat(orders + 1, 2)
  return features / denominator if rescale else features


prior_base_d = tfd.Normal
...
def make_likelihood_model(
  ...
  if LikelihoodDist(distribution) == LikelihoodDist.NORMAL:
    ...
    # print("log_noise_scale", log_noise_scale)
...
class BayesianNeuralField1D(nn.Module):
  ...
  frequency_feature_index: int = 1
  target_coord_indices: Tuple[int, int] = (2, 3)
  reference_feature_start: int = 4
  reference_feature_stride: int = 3
  num_reference_points: int = 0
  use_physical_guidance: bool = True
  time_fourier_order: int = 5
  frequency_fourier_order: int = 5
  time_fourier_scale: float = 2 * np.pi
  frequency_fourier_scale: float = 2 * np.pi

  def _physical_guidance_features(self, x, scaled_x):
    """Builds s_k exp(-gamma d_k) features from sparse sensors."""
    if not self.use_physical_guidance or self.num_reference_points <= 0:
      return []

    input_scales = jnp.asarray(self.input_scales)
    ref_offsets = (
        self.reference_feature_start
        + self.reference_feature_stride * np.arange(self.num_reference_points)
    )
    signal_idx = ref_offsets
    dx_idx = ref_offsets + 1
    dy_idx = ref_offsets + 2

    signals = jnp.take(scaled_x, signal_idx, axis=-1)
    dx = jnp.take(x, dx_idx, axis=-1) / jnp.take(input_scales, dx_idx)
    dy = jnp.take(x, dy_idx, axis=-1) / jnp.take(input_scales, dy_idx)
    distances = jnp.sqrt(jnp.square(dx) + jnp.square(dy) + 1e-6)

    gamma = jax.nn.softplus(
        self.param(
            'physical_gamma_unconstrained',
            nn.initializers.constant(0.0),
            (),
        )
    ) + 1e-6
    decay = jnp.exp(-gamma * distances)
    weighted_signals = signals * decay
    weighted_sum = jnp.sum(weighted_signals, axis=-1, keepdims=True)
    decay_sum = jnp.sum(decay, axis=-1, keepdims=True)
    weighted_mean = weighted_sum / (decay_sum + 1e-6)
    return [signals, distances, decay, weighted_signals, weighted_mean]
  ...
  def __call__(self, x):
    ...
    time_features = make_multiscale_fourier_features(
        scaled_x[..., self.time_feature_index],
        self.time_fourier_order,
        self.time_fourier_scale,
        rescale=True,
    )
    frequency_features = make_multiscale_fourier_features(
        scaled_x[..., self.frequency_feature_index],
        self.frequency_fourier_order,
        self.frequency_fourier_scale,
        rescale=True,
    )
    ...
    base_idx = np.asarray(
        [self.time_feature_index, self.frequency_feature_index], dtype=int
    )
    target_coord_idx = np.asarray(self.target_coord_indices, dtype=int)
    base_features = jnp.take(scaled_x, base_idx, axis=-1)
    target_coords = jnp.take(scaled_x, target_coord_idx, axis=-1)
    physics_features = self._physical_guidance_features(x, scaled_x)
